...
function normalizeLocalCard(product: any, mediaById: Map<string, any> = new Map()): ProductCard {
  ...
  return {
    ...
    sellerName: product.seller?.sellerProfile?.shop_name || product.seller?.email || 'Matrix Shop',
    vendorName: product.vendor_name || product.seller?.sellerProfile?.shop_name || product.seller?.email || 'Matrix Shop',
    vendorId: product.seller_id,
    shopName: product.seller?.sellerProfile?.shop_name || product.seller?.email || 'Matrix Shop',
...
export async function getVendorInfo(vendorId: string) {
  ...
  return {
    ...
    email: seller.email,
    phone: seller.phone,
    shopName: seller.sellerProfile?.shop_name || seller.email || 'Matrix Shop',
    displayName: seller.sellerProfile?.display_name || seller.sellerProfile?.shop_name || seller.email || 'Matrix Shop',
