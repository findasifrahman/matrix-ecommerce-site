  ...
  <div class="min-h-screen bg-slate-50 flex">
    ...
    <aside class="hidden lg:block w-64 bg-teal-600 border-r border-teal-700 fixed h-screen overflow-y-auto">
      <div class="p-4 border-b border-teal-700">
        <router-link to="/shopping" class="flex items-center space-x-2 mb-2">
          <span class="text-xl font-bold text-white">MatrixShop</span>
        </router-link>
        <h2 class="text-sm font-semibold text-teal-100">Customer Account</h2>
      </div>
      ...
      <header class="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div class="px-4 sm:px-6 py-4 flex justify-between items-center">
          ...
          <div class="flex items-center space-x-4">
            <router-link
              to="/shopping"
              class="hidden items-center gap-2 text-slate-700 transition-colors hover:text-teal-600 sm:flex"
            >
              <Home class="h-5 w-5" />
    ...
    <Drawer v-model="mobileMenuOpen" position="left" width="xs" title="Menu">
      <nav class="flex flex-col space-y-2 p-4">
        <router-link @click="mobileMenuOpen = false" to="/shopping" class="flex items-center gap-3 px-3 py-2 rounded-md text-base font-medium text-slate-700 hover:bg-slate-100">
          <Home class="h-5 w-5" />
          <span>Shop Home</span>
        </router-link>
